...
const AddCardsAgendaScreen = () => {
  ...
  const fetchData = async () => {
    const myCards = await connector.db.card.getMyCards(userID)
    ...
    const cardsWithID = uniqueCards.reduce((acc, card) => ({ ...acc, [card.id]: card }), {})
  ...
  useEffect(() => {
    ...
    fetchAgenda()
    fetchData().finally(() => store.setLoading(false))
  }, [isFocused])
  ...
  const addCardToAgenda = () => {
    if (!selectedCard) return console.error('No card or day selected')

    const newAgendaSequence = [
      ...(agenda[selectedDay ?? 0] || []),
      { type: 'card', uniqueId: getUUID(), content: selectedCard },
    ]

    connector.db.agenda.setDay(selectedDay ?? 0, newAgendaSequence, userID).then((success) => {
      if (!success) return console.error('Failed to add card to agenda')

      setAgenda({
        ...agenda,
        [selectedDay]: newAgendaSequence,
      })
  ...
  return (
    <ScreenLayout disableMargin backgroundSecondary tabBar={false}>
      <Appbar.Header statusBarHeight={0} className="bg-secondary">
        ...
        <Appbar.Content
          title={t('screens.agenda.addCardsAgendaScreen.title')}
          color="black"
      ...
      <View className="flex-1 rounded-t-3xl bg-white p-4">
        <SafeAreaView className="flex-1">
          ...
          <Text className="mb-2 ml-2 mt-4 text-lg font-bold">Saved Cards</Text>
      ...
      <Portal>
        ...
        >
          <Text className="mb-4 text-xl font-bold">{t('screens.agenda.addCardsAgendaScreen.addItem')}</Text>
          ...
          <View className="h-10 w-full flex-row justify-around">
            ...
            <Button mode="contained" className="mr-2 rounded-lg bg-primary px-2" onPress={addCardToAgenda}>
              {t('standards.addCard')}
            </Button>
          </View>
        </Modal>
...
export default AddCardsAgendaScreen
